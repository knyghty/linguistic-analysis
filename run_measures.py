...
for chain_id, chain in parsed_chains.items():
    ...
    lilla_scores = silla_scores = lsm_scores = wv_scores = []
    for counter, message_a in enumerate(messages_a, 1):
        for message_b in messages_b[counter:]:
            lilla_scores.append(lla.lilla(message_a['textblob'].words, message_b['textblob'].words))
            silla_scores.append(lla.silla(message_a['spacy'], message_b['spacy']))
            lsm_scores.append(lsm.dyad_lsm(message_a['spacy'], message_b['spacy']))
            wv_scores.append(message_a['spacy'].similarity(message_b['spacy']))
